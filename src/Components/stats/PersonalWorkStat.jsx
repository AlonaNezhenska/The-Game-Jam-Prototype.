function PersonalWorkStat({ personalWorkStat, setPersonalWork }) {
    ...
    // Each item gets disabled when learning score reaches a certain level
    const personalWorkScore = personalWorkStat;
    ...
    const createGridItems = () => {
        ...
        for (let i = 0; i < 5; i++) {
            const isDisabled = personalWorkScore < disableThresholds[i][0];
            const isRangeActive = personalWorkScore >= disableThresholds[i][0];
            const itemStyle = {
            ...
            items.push(
                <div
                    key={i}
                    style={itemStyle}
                    // onClick={() => !isDisabled && setPersonalWork(personalWorkStat + 1)}
                >
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', margin: '0 10px' }}>
            <h3 style={{ 
                ...
                Personal Work: {personalWorkStat}
            </h3>
            <div style={{
                display: 'grid',
...
export default PersonalWorkStat;
